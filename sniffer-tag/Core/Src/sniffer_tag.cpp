...
void reset_TAG_values(TAG_t *tag) {
	...
	tag->distance_a = 0;
	tag->distance_b = 0;
	tag->Float_Battery_Voltage = 0.0;
	tag->Real_Batt_Voltage  = 0;
 ...
 * Formato optimizado: muestra solo primeros/últimos errores cuando hay muchos repetidos
 * @param buffer Puntero al buffer de logging
 */
void log_buffer_print(LogBuffer_t *buffer) {
    ...
    // Contar errores por antena y tipo
    uint8_t errors_ant_a = 0;
    uint8_t errors_ant_b = 0;
    uint8_t timeout_count = 0;
    uint8_t crc_errors = 0;
    uint8_t other_errors = 0;
    ...
    for (uint8_t i = 0; i < buffer->count; i++) {
        ...
        if (event->result == TAG_RX_PREAMBLE_DETECTION_TIMEOUT || 
            event->result == TAG_RX_FRAME_TIMEOUT) {
            timeout_count++;
        } else if (event->result == TAG_NO_RXCG_DETECTED) {
            crc_errors++;
        } else {
            other_errors++;
        }
    ...
    snprintf(log_line, sizeof(log_line), 
             "\r\n╔═══ ERROR LOG: Tag 0x%08X ═══╗\r\n", 
             (unsigned int)buffer->tag_id);
    HAL_UART_Transmit(&huart1, (uint8_t*)log_line, strlen(log_line), HAL_MAX_DELAY);
    
    snprintf(log_line, sizeof(log_line), 
             "║ Total Errors: %d (Ant-A:%d, Ant-B:%d)\r\n", 
             buffer->count, errors_ant_a, errors_ant_b);
    HAL_UART_Transmit(&huart1, (uint8_t*)log_line, strlen(log_line), HAL_MAX_DELAY);
    
    snprintf(log_line, sizeof(log_line), 
             "║ Types: Timeouts=%d, CRC=%d, Others=%d\r\n", 
             timeout_count, crc_errors, other_errors);
    HAL_UART_Transmit(&huart1, (uint8_t*)log_line, strlen(log_line), HAL_MAX_DELAY);
    
    snprintf(log_line, sizeof(log_line), "╠═════════════════════════════════════════╣\r\n");
    HAL_UART_Transmit(&huart1, (uint8_t*)log_line, strlen(log_line), HAL_MAX_DELAY);
    
    // Si hay muchos errores (>10), mostrar solo primeros 3, últimos 3 y resumen
    const uint8_t MAX_SHOW = 10;
    const uint8_t SHOW_FIRST = 3;
    const uint8_t SHOW_LAST = 3;
    
    if (buffer->count > MAX_SHOW) {
        // Mostrar primeros 3
        for (uint8_t i = 0; i < SHOW_FIRST; i++) {
            LogEvent_t *event = &buffer->events[i];
            const char* antenna_name = (event->antenna == 0) ? "Ant-A" : "Ant-B";
            ...
            snprintf(log_line, sizeof(log_line),
                    "║ %s [%s] %s (R:%d, %lums)\r\n",
                    indicator, antenna_name, result_name, 
                    event->reading_counter, event->elapsed_ms);
            
            HAL_UART_Transmit(&huart1, (uint8_t*)log_line, strlen(log_line), HAL_MAX_DELAY);
        }
        
        // Indicador de omisión
        uint8_t omitted = buffer->count - SHOW_FIRST - SHOW_LAST;
        snprintf(log_line, sizeof(log_line),
                "║ ... (%d similar errors omitted) ...\r\n", omitted);
        HAL_UART_Transmit(&huart1, (uint8_t*)log_line, strlen(log_line), HAL_MAX_DELAY);
        
        // Mostrar últimos 3
        for (uint8_t i = buffer->count - SHOW_LAST; i < buffer->count; i++) {
            LogEvent_t *event = &buffer->events[i];
            const char* antenna_name = (event->antenna == 0) ? "Ant-A" : "Ant-B";
        ...
        // Mostrar todos los errores (formato compacto en una línea)
        for (uint8_t i = 0; i < buffer->count; i++) {
            LogEvent_t *event = &buffer->events[i];
            const char* antenna_name = (event->antenna == 0) ? "Ant-A" : "Ant-B";
...
// Allocate buffer for serialized tags
	uint8_t buffer[SERIALIZED_TAG_SIZE_ONE_DETECTION * list->count] = {0};
	serialize_tag_list_limit_od(list, buffer, list->count, _total_tags, _sniffer_id);

// Print each serialized tag in hexadecimal format separated by "\n"
	uint8_t *current = buffer+6;
	char hex_output[SERIALIZED_TAG_SIZE_ONE_DETECTION * 2 + 1+8+2+2] = {0}; // Each byte is represented by 2 hex digits + null terminator
	...
	for (int i = 0; i < list->count; ++i) {
		for (uint32_t j = 0; j < SERIALIZED_TAG_SIZE_ONE_DETECTION; ++j) {
			snprintf(&hex_output[j * 2], 3, "%02X", current[j]);
		}

		HAL_UART_Transmit(&huart1, (uint8_t*) hex_output,
				(uint16_t) (SERIALIZED_TAG_SIZE_ONE_DETECTION * 2), HAL_MAX_DELAY);
		HAL_UART_Transmit(&huart1, (uint8_t*) "\n\r\r", 2,
		HAL_MAX_DELAY);
		current += SERIALIZED_TAG_SIZE_ONE_DETECTION;
	}
	HAL_UART_Transmit(&huart1, (uint8_t*) "####\n\r", (uint16_t) 6,
	HAL_MAX_DELAY);
}
...
uint8_t saveTagIfNeeded(TAG_t *tag, DistanceHandler *distance_a,
