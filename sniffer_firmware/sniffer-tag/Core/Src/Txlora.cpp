...
Txlora::Txlora(Gpio _nss, Gpio _reset, SPI_HandleTypeDef *_spi, Memory* _eeprom) :
	Txsx1278(_nss, _reset, _spi) {
...
int8_t Txlora::receive(uint8_t *data_received, LinkMode mode) {
...
void Txlora::set_link_frequency(LinkMode mode) {
...
uint8_t Txlora::transmit(uint8_t *data, uint8_t data_len, LinkMode mode) {
...
void Txlora::set_lora_settings(LoraBandWidth bw, CodingRate cr, SpreadFactor sf,
...
void Txlora::set_low_frequency_mode(DeviceOperatingMode mode) {
...
void Txlora::set_default_configurations() {
	set_low_frequency_mode(DeviceOperatingMode::SLEEP);
