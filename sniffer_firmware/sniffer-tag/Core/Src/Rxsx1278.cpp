...
Rxsx1278::Rxsx1278(Gpio _nss, Gpio _reset, SPI_HandleTypeDef *_spi) {
	nss = _nss;
	reset = _reset;
...
uint8_t Rxsx1278::read_reg_addr(LoraRegisters reg, uint8_t reg_len) {
	if(reg_len <= 0 )
		return -1;
...
void Rxsx1278::write_reg_addr(uint8_t address, uint8_t *cmd, uint8_t lenght) {
	if (lenght > 4)
		return;
	uint8_t tx_data[5] = { 0 };
...
uint8_t Rxsx1278::read_8bit_reg(LoraRegisters reg) {
	uint8_t reg_len = 1;
...
void Rxsx1278::write_8bit_reg(LoraRegisters reg, uint8_t _value) {
	uint8_t value[1];
	value[0] = _value;
	write_reg_addr(static_cast<uint8_t>(reg), value, 1);
}

void Rxsx1278::set_carrier_frquency(uint32_t frequency) {
...
void Rxsx1278::setRegModemConfig(uint8_t modem_cfg1, uint8_t modem_cfg2) {
...
void Rxsx1278::setDetectionParametersReg() {
	uint8_t tmp;
...
int8_t Rxsx1278::wait_irq(uint8_t mask, uint16_t timeout) {
...
uint8_t Rxsx1278::write_tx_fifo_data(uint8_t *data, uint8_t data_len) {
	if (data_len > 0) {
