...
Lora::Lora(Gpio nss, Gpio reset, SPI_HandleTypeDef *_spi, Memory* _eeprom) :
	Sx1278(nss, reset, _spi) {
...
uint8_t Lora::receive(uint8_t *data_received, LINKMODE mode) {
	...
	if (!wait_irq(RX_DONE_MASK, 3000)) {
		uint8_t rx_nb_bytes = read_8bit_reg(LoraRegisters::RegRxNbBytes); //Number for received bytes
		uint8_t point_current = read_8bit_reg(LoraRegisters::RegFifoRxCurrentaddr);
		write_8bit_reg(LoraRegisters::RegFifoAddrPtr, point_current);
